    ...
    ActionHandler = class ActionHandler extends coreModule.api.ActionHandler {
        ...
        */
        async #buildHeroActions () {
            if (!game.modules.get('sf2e-hero-actions')?.active) return

            const actionType = 'heroAction'
            const actionTypeName = coreModule.api.Utils.i18n(ACTION_TYPE[actionType])
            const heroActions = this.actor.getFlag('sf2e-hero-actions', 'heroActions') ?? []
            ...
            if (remainingHeroPoints > 0) {
                actions.push({
                    id: 'drawHeroActions',
                    name: game.i18n.format('sf2e-hero-actions.templates.heroActions.draw', { nb: remainingHeroPoints }),
                    listName: `${actionTypeName}: ${game.i18n.localize('sf2e-hero-actions.templates.heroActions.draw').replace('({nb}) ', '')}`,
        ...
        async #buildInitiative () {
            ...
            const skillActions = await Promise.all(
                [...coreSkills, ...loreSkills].map(async ([skillId, skillData]) => {
                    ...
                    const fullName = coreModule.api.Utils.i18n(data.label) ?? coreModule.api.Utils.i18n(CONFIG.SF2E.skillList[skillId])
        ...
        #buildRests () {
            ...
            // Take a Breather
            if (game.settings.get('sf2e', 'staminaVariant')) {
                actions.push({
                    id: 'takeBreather',
        ...
        async #buildSaves () {
            ...
            // Get saves
            const saves = this.actor ? Object.entries(this.actor.saves || []) : Object.entries(CONFIG.SF2E.saves)
        ...
        // Skill action macros not yet implemented in sf2e
        /*
         * Build skill actions
         * @private
         */
        ...
        */
            ...
            for (const [key, value] of skillsMap) {
                ...
                const actions = await Promise.all(
                    [...skills].map(async ([skillId, skillData]) => {
                        const id = skillId
                        const label = coreModule.api.Utils.i18n(skillData.label) ?? coreModule.api.Utils.i18n(CONFIG.SF2E.skillList[skillId])
        ...
        // Elemental blast not present in sf2e
        /**
